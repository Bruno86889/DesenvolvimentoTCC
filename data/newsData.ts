export const newsData: NewsData[] = [
    {
        data: '27/11/2022',
        titulo: 'Festival de Dança está de volta!',
        subtitulo: 'O evento volta em sua 7ª Edição e ocorrerá nos dias 8 e 9 de dezembro a partir das 19h',
        src: "festival.jpg",
        url: 'blog/vii-festival-de-danca',
    },
    {
        data: '18/11/22',
        titulo: 'Semana da Conciência Negra',
        subtitulo: 'A semana conta com palestras de Mauricío Moyses e Julia Motta',
        src: "scnh.jpg",
        url: 'blog/semana-da-conciencia-negra',
    },
    {
        data: '17/11/22',
        titulo: '28ª SIPAT',
        subtitulo: "O Evento ocorrerá nos dias 21, 22 e 23 de Novembro a partir das 19h",
        src: "sipat.jpg",
        url: 'blog/xxiix-sipat'
    },
    {
        data: '04/11/22',
        titulo: 'Concurso público',
        subtitulo: "Estão abertas as incrições para os concursos públicos de professores e axiliares administrativos",
        src: "concursopublico.jpg",
        url: 'blog/concurso-publico-professor'
    },
    {
        data: '03/11/22',
        titulo: 'Venha conhecer nossa ETEC no ETEC portas abertas',
        subtitulo: "O evento tem como objetivo divulgar as atividades da escola para os pais e responsáveis",
        url: 'blog/etec-portas-abertas',
        src: "etecportasabertas.jpg"
    },
]